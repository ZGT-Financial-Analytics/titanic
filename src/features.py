...
def normalize_title(title: str) -> str:
    title = title.strip()
    common = {"Mr","Mrs","Miss","Master","Dr","Rev","Col","Mlle","Mme","Ms","Major","Lady","Sir","Don","Jonkheer","Capt","Countess","Dona"}
    if title in {"Mlle"}: return "Miss"
    if title in {"Ms"}: return "Miss"
    if title in {"Mme"}: return "Mrs"
    if title in {"Lady","Countess"}: return "Noble"
    if title in {"Sir","Don","Dona","Jonkheer"}: return "Noble"
    if title in {"Col","Major","Capt"}: return "Officer"
